...
impl fmt::Display for TransomeError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            TransomeError::ModelNotFound { model_name, available_models } => {
                write!(
                    f,
            ...
            TransomeError::ApiCallFailed { endpoint, status_code, message } => {
                match status_code {
                    Some(code) => write!(
                        f,
            ...
            TransomeError::ValidationError { field, expected, actual } => {
                write!(
                    f,
...
impl TransomeError {
    ...
    pub fn user_friendly_message(&self) -> String {
        match self {
            ...
            TransomeError::ApiCallFailed { endpoint: _, status_code, message } => {
                match status_code {
            ...
            TransomeError::ValidationError { field, expected, .. } => {
...
mod tests {
    ...
    fn test_error_type_checks() {
        ...
        let general_error = TransomeError::General { message: "test".to_string() };
